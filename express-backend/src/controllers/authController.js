...
const { logFailedLogin } = require('../helpers/loginLogger')
const {
    ...
    try {
      ...
      if (data.length === 1) {
        ...
        if (pass) {
          ...
          const shouldBlock = await logFailedLogin({ email, timestamp: new Date() });
          if (shouldBlock) {
            return response(res, 'Too many failed login attempts. Please try again tomorrow.', 403, false);
          }
